...
* 跟随系统、浅色和深色外观；Rift、Ocean、Ember 配色；减少动态效果。偏好设置使用 DataStore 保存。
* 支持设为 Android 默认桌面。进入 **设置 → 主屏幕启动器 → 设为默认桌面**，由 Android 请求确认；取消后保留原有默认桌面。
...
ZIP 文件必须只包含一个 GBA ROM。应用会拒绝不安全的路径、无法明确选择 ROM 的压缩包和超大文件。解压后的 ROM 存放于私有临时缓存，通过 FileProvider 授予有限的读取权限；前端恢复运行或准备下一个 ROM 时，会清理已超过 24 小时的旧缓存文件。
...
元数据刮削、视频预览、更多模拟器专用适配器和 Android 应用抽屉仍在计划中。
...
Media3 视频预览、Baseline Profiles 和宏基准测试仍在计划中。目前没有原生核心；只有性能分析证明有必要时，才会考虑引入 C++。
...
应用框架、本地 GBA 游戏库和首个外部模拟器启动流程已实现。后续工作重点包括目标掌机验证、元数据与封面、可选的视频预览、性能分析，以及更多游戏平台支持。
